...
public static class ProductEndpoints
{
    ...
    private static async Task<Ok<PagedResult<ProductDto>>> GetProductsAsync(AppDbContext db, int page = 1, int pageSize = 20, string? search = null)
    {
    ...
    private static async Task<Results<Created<ProductDto>, Conflict<ErrorResponse>>> CreateProductAsync(CreateProductRequest request, AppDbContext db)
    {
        ...
        if (skuExists)
        {
            return TypedResults.Conflict(new ErrorResponse("A product with this SKU already exists."));
        }
    ...
    private static async Task<Results<Ok<ProductDto>, NotFound>> UpdateProductAsync(Guid id, UpdateProductRequest request, AppDbContext db)
    {

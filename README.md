...
- **A refresh-token delivery strategy for browser clients** — see the note in the
  frontend/React guidance for why an httpOnly cookie is usually better than putting
  the refresh token in a JSON response body once a browser is involved.
